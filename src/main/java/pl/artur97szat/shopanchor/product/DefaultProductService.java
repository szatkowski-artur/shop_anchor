...
//    public ShopType ch
    ...
    public void saveProduct(AddProductDto addProductDto) {
        ...
        if (ShopType.existByUrl(url))
            product.setShop(shopService.getShopByType(ShopType.getByUrl(url)));

        try {
            product.setAvailable(checkIfAvailable(product));
    ...
    public Boolean checkIfAvailable(Product product) throws CouldNotConnectToPageException {
        ...
        try {
            ...
            log.error("Can't connect with URL", e);
